    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {['gold', 'silver', 'bronze', 'purple', 'red'].map(categoria => (
          <div key={categoria} className="flex items-center space-x-1">
            <div className={`w-3 h-3 rounded-full ${getCategoryColor(categoria).split(' ')[0]}`}></div>
            <span className="text-xs capitalize">{categoria}</span>
          </div>
        ))}
      </div>
      ...
      >
        ...
        {tables.map((table) => (
          <Draggable
            key={table.id}
            position={{ x: table.x || 50, y: table.y || 50 }}
            onStop={(e, data) => handleDrag(table.id, data)}
            bounds="parent"
          >
            <div
              className={`absolute cursor-move select-none transition-all duration-200 ${
                selectedTable === table.id ? 'scale-110 z-10' : 'z-0'
              }`}
            ...
            >
              ...
              {selectedTable === table.id && (
                <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 bg-white border rounded-lg shadow-lg p-3 min-w-[200px] z-20">
                  ...
                  <div className="text-xs text-gray-600 space-y-1">
                    ...
                    <div>
                      <Badge 
                        variant={table.disponible ? "default" : "secondary"}
                        className="text-xs"
                      >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-4">
        {['gold', 'silver', 'bronze', 'purple', 'red'].map(categoria => {
          ...
          return (
            <div key={categoria} className="text-center p-3 border rounded-lg">
